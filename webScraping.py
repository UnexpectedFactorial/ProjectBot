...
class Wiki(Scrape):
  def scrapeFromWiki(self,page):
    if self.headerCheck():
      ...
      for div in site.find_all('div', class_='mw-body-content mw-content-ltr'):
        return(site.find_all('p')[1].text)  
    elif self.headerCheck() == 404:
      return "Sorry, this page does not exist! Please check the term for any spelling mistakes!"
    else:
  ...
  def disambigCheck(self):
    pass

  def wikiScrape(self,choice):
    page = requests.get(self.url,headers=self.header)
    site = BeautifulSoup(page.content, "html.parser")
